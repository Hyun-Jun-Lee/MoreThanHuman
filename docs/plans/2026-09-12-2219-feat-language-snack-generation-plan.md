...
Q1–Q4는 구현 시작 요청 시 권장 기본값으로 안내하고 적용한 항목이며 개별 답변을 받았다는 의미는 아니에요. 운영 DB migration·실제 생성·cron 등록은 실행하지 않았고 README 배포 절차로 남겨요.
...
- 운영 DB migration, 실제 LLM 표본의 사실 검증, Docker 이미지 배포, 서버 cron 등록, 실제 기기 시각 검수는 아직 실행하지 않았어요. README의 배포·롤백 절차를 따라 진행해요.

문서 내 일관성과 실행 가능성을 직접 검토했어요. 언어별 조회 요구에 맞춰 기존 공통 목록 정책, 캐시 분리, run_key의 언어 단위 및 테스트를 함께 수정했어요.
구현 기본값은 Applied Defaults에 기록했어요. 상세 계약·환경변수·내부 구조는 각각 docs/DSL.md, README.md, .agent/architecture.md에 반영했어요. 운영 배포 확인이 남아 있어 이 문서는 이행 기록으로 유지해요. 독립 다중 에이전트 리뷰 대신 현재 세션에서 직접 검토했고 실제 LLM 품질·운영 배포 smoke는 아직 수행하지 않았어요.
