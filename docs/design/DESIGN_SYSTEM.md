...
**Tomato Language Snacks (v2.3, 2026-09-19)**

- Home의 최근 대화 위에 section label, 현재 열람 수/12와 토마토 바구니를 표시해요. 투명 PNG 바구니와 수정된 3입 이미지를 사용하고 별도 배경 없이 Home 배경 위에 표시해요. 텍스트·컨트롤은 onSurface 색상을 사용해요.
...
- 토마토 번호/3은 유지하고 하단 4입 진행 점은 표시하지 않아요. 바구니의 3개 완료 표시는 유지하며 전환 시 높이가 바뀌지 않게 같은 여백을 확보해요. 터치 도중과 콘텐츠 팝업이 열렸을 때 중복 입력을 막고, reduce motion에서는 토마토 움직임을 생략해요. 바구니 아이콘과 토마토 버튼에 툴팁·접근성 이름을 제공해요.
